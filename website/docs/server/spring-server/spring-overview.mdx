...
With Spring Boot 4, Jackson 3 is auto-configured by Spring Boot and `graphql-kotlin-spring-server` uses that configuration.

- Prefer configuring JSON through `spring.jackson.*` properties.
- For programmatic customization, provide a `JsonMapperBuilderCustomizer` bean.
- Avoid defining an additional standalone `ObjectMapper` bean unless you intentionally want separate mapper instances.
