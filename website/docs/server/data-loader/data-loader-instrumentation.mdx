...
- In the data source, let's say a database, an `Astronaut` can have multiple `Mission`s and a `Mission` can have multiple `Planet`s.
- The GraphQL Schema exposes the `Astronaut` `Planet`s, without traversing his `Mission`s.
...
- The data fetchers (resolvers) will contain the logic to collect missions by astronaut, and planets by missions.
...
to chain and still apply batching and deduplication to a chained `DataLoader` in your `DataFetcher` or `Resolver`.
