...
There are some use cases where a GraphQL Schems doesn't match the datasource schema, a field can require data from multiple
data sources to be fetched.
...
In order to access to an `Astronaut` planets we would need to retrieve all the `Astronaut` `Mission`s and
for each `Mission` retrieve all the `Planet`s.

The problem with this schema is that in order to access to the `Astronaut` `Planet`s we would need to
access to his `Mission`s first.

Instead we could have an schema like this:
...
This `GraphQL Schema` allows to access to an `Astronaut` `Planet`s without traversing his `Missions`, so we can just focus
on the logic to chain `DataLoaders` as we would still need to access to the `Astronaut` `Planet`s through his `Mission`s.
...
```kotlin
class Astronaut {
    fun getPlanets(astronautId: Int, environment: DataFetchingEnvironment): CompletableFuture<List<Planet>> {
...
`graphql-kotlin-dataloader-instrumentation` includes a helpful extension function on the `CompletableFuture`
so that you can easily instruct the [previously selected data loader instrumentation](./data-loader-instrumentation#dispatching-by-level) that you want
to chain and still apply batching and deduplication to a chained `DataLoader`.
