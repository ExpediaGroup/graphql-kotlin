...
This function accepts five arguments: `config`, `queries`, `mutations`, `subscriptions` and a `schemaObject`. The `queries`, `mutations`
and `subscriptions` are a list of `TopLevelObject`s and will be used to generate corresponding GraphQL root types. `schemaObject` is an
optional `TopLevelObject` reference to the annotated schema object. See below on why we use this wrapper class. The `config` contains all
the extra information you need to pass, including custom hooks, supported packages, and name overrides. See the [Generator Configuration](./customizing-schemas/generator-config.md)
