...
class GraphQLKtorClientTest {
    ...
    fun `verifies ktor client can retrieve data`() {
        val expectedResponse = GraphQLResponse<HelloWorldResult>(
            ...
            errors = listOf(
                GraphQLError(
                    ...
                    path = listOf(JsonPrimitive("helloWorld")),
                    extensions = mapOf("exceptionExtensionKey" to JsonPrimitive("JunitCustomValue"))
                )
            ),
            extensions = mapOf("extensionKey" to JsonPrimitive("JUnitValue"))
        )
        ...
        runBlocking {
            ...
            assertEquals(JsonPrimitive("helloWorld"), actualError.path?.firstOrNull())
            assertEquals(JsonPrimitive("JunitCustomValue"), actualError.extensions?.get("exceptionExtensionKey"))
            assertNotNull(result.extensions)
            assertEquals(JsonPrimitive("JUnitValue"), result.extensions?.get("extensionKey"))
        }
    }
    ...
    private inline fun <reified R> stubResponse(response: GraphQLResponse<R>, delayMillis: Int = 0): MappingBuilder =
        WireMock.post("/graphql")
            .willReturn(
                WireMock.aResponse()
                    .withStatus(200)
